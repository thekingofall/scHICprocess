...
#
# 选项:
#   -c <配置文件>         HiC-Pro配置文件路径（默认: /home/maolp/mao/Codeman/Project/DIPC/scCARE.txt）
...
output_dir="Run3_hic"
log_dir="Run0_log"
default_config="/home/maolp/mao/Codeman/Project/DIPC/scCARE.txt"
default_hicpro="/home/maolp/mao/Biosoft/HiC-Pro-3.1.0/bin/HiC-Pro"
default_sentenmail="/home/maolp/mao/Codeman/All_Archived_Project/SentEmail.py"
default_summary_script="/home/maolp/mao/Codeman/Project/DIPC/scCARE-seq/Processing_Hi-C/hicpro_summary_trans.pl"
default_juicebox_script="/home/maolp/mao/Biosoft/HiC-Pro-3.1.0/bin/utils/hicpro2juicebox.sh"
default_juicer_tools="/home/maolp/mao/Biosoft/juicer_tools_1.22.01.jar"
default_genome_sizes="/home/maolp/mao/Ref/AllnewstarRef/Homo/HG19/hg19.sizes"
default_genome_bed="/home/maolp/mao/Ref/AllnewstarRef/Homo/HG19/HG19mboi.bed"
config_file="$default_config"
cpu=10
conda_env="hicpro3"
HiC_Pro_path="$default_hicpro"
SentEmail_path="$default_sentenmail"
SummaryScript_path="$default_summary_script"
JuiceboxScript_path="$default_juicebox_script"
JuicerTools_path="$default_juicer_tools"
GenomeSizes_path="$default_genome_sizes"
GenomeBed_path="$default_genome_bed"
...
usage() {
    ...
    echo
    echo "选项:"
    echo "  -c <配置文件>         HiC-Pro配置文件路径（默认: $default_config）"
    ...
    echo "  --HiC-Pro <路径>      HiC-Pro可执行文件路径（默认: $default_hicpro）"
    echo "  --SentEmail <路径>    SentEmail.py脚本路径（默认: $default_sentenmail）"
    echo "  --SummaryScript <路径> hicpro_summary_trans.pl脚本路径（默认: $default_summary_script）"
    echo "  --JuiceboxScript <路径> hicpro2juicebox.sh脚本路径（默认: $default_juicebox_script）"
    echo "  --JuicerTools <路径>    Juicer_tools.jar路径（默认: $default_juicer_tools})"
    echo "  --GenomeSizes <路径>    hg19.sizes文件路径（默认: $default_genome_sizes})"
    echo "  --GenomeBed <路径>      hg19mboi.bed文件路径（默认: $default_genome_bed})"
    echo "  -h                    显示此帮助信息"
    echo
    exit 1
}
...
# 解析命令行选项
while getopts ":c:p:i:t:o:j:u:e:-:" opt; do
    case ${opt} in
        c )
            config_file="$OPTARG"
            ;;
...
conda activate "$conda_env" || { echo "错误: 无法激活Conda环境 '$conda_env'"; exit 1; }
